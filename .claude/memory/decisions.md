...
### HTTPS con Caddy sulla porta 3000, certificato via DNS-01 DuckDNS

**Data:** 2026-09-24
**Decisione:** Caddy (`deploy/caddy/`, immagine costruita sul Pi con il modulo `caddy-dns/duckdns`) prende la **porta host 3000** con TLS e fa reverse proxy verso `timesheet:3000`, che non è più pubblicato sull'host. URL: `https://myservergio.duckdns.org:3000`. Certificato Let's Encrypt con challenge **DNS-01** (token `DUCKDNS_TOKEN` nel `.env` del Pi, mai nel repo); `http://` sulla stessa porta riceve un 308 verso `https://` (listener wrapper `http_redirect`). `COOKIE_SECURE: "false"` rimosso: in produzione il cookie torna `Secure`.
**Perché:** Il riconoscimento vocale del form (`webkitSpeechRecognition`) richiede un secure context: su HTTP il browser nega il microfono. Chiude anche il debito "login in chiaro su internet". Porta 3000 riusata perché il router la inoltra già al Pi: **nessuna modifica al router**. DNS-01 perché la porta 80 del Pi è di `casaos-gateway` e così il rinnovo non dipende da nessun port-forward. In LAN non serve un record DNS locale: i client usano il router come DNS e l'**hairpin NAT funziona** (verificato).
**Alternative scartate:** challenge TLS-ALPN sulla 443 — richiedeva un nuovo port-forward sul router; immagine Caddy di terzi con il modulo DuckDNS — scartata perché il container tiene il token e termina il TLS; record DNS locale su Pi-hole — inutile, Pi-hole non è il DNS dei client.
**Impatto:** `docker-compose.yml` (servizio `caddy`, volumi `caddy-data`/`caddy-config` per non richiedere il certificato a ogni ricreazione), `deploy/caddy/Dockerfile`, `deploy/caddy/Caddyfile`, `.env.example`. Sul Pi il compose **non** è una copia di quello del repo (ha il default `IAPI_BASE_URL=http://192.168.1.50:8000` che il `.env` del Pi non definisce): va modificato, non sostituito. Supera la decisione sul cookie non-Secure qui sotto.

### Cookie di sessione non-Secure su `myservergio.duckdns.org` (istanza HTTP, senza TLS davanti) — **superata il 2026-09-24**
...
### Ore oltre le 8h: la scelta si chiede al salvataggio, e lo straordinario non conta nelle ordinarie

**Data:** 2026-09-25
**Decisione:** Quando una voce di **lavoro** (SUPPORTO, MANUTENZIONE) porta le ore **ordinarie** del giorno oltre `MINUTES_PER_WORKDAY` (480), il salvataggio si ferma e l'utente sceglie: **straordinario** (la voce si divide: la parte entro le 8h resta del tipo scelto, l'eccedenza diventa una voce `STRAORDINARIO` nello stesso giorno) oppure **spostamento** (l'eccedenza va sui giorni lavorativi successivi, riempiendo ciascuno fino a 8h e proseguendo a cascata se il primo non basta, come richiesto esplicitamente dall'utente; saltati weekend, festivi e giorni pieni). Ore ordinarie = tutte le voci **tranne `STRAORDINARIO`**: ferie e permessi coprono la giornata, lo straordinario per definizione ne sta fuori. In **modifica** si chiede solo se la modifica **aumenta** l'eccedenza del giorno.
**Perché:** Contare lo straordinario tra le ordinarie farebbe scattare la domanda su ogni voce successiva dello stesso giorno, anche dopo che l'utente ha già scelto. La regola "solo se aumenta" evita che correggere il titolo di una voce in un giorno già oltre le 8h (es. per ferie inserite dopo) riapra la domanda. La decisione è **del server**: `createTimeEntry`/`updateTimeEntry`/`logTaskAsEntry` restituiscono `{ ok: false, overflow }` senza scrivere nulla (nemmeno cliente/progetto/tag), il client mostra `OverflowPrompt` e richiama la stessa action con la scelta. Il pannello è visibile solo finché data/durata/tipo del form coincidono con quelli per cui è stato calcolato, e la scelta risottomette i valori correnti: il server ricalcola sempre, quindi niente dati stantii.
**Alternative scartate:** calcolare l'eccedenza nel client — richiederebbe di caricare le ore dei giorni successivi nella pagina e lascerebbe la regola aggirabile; dialog modale — i punti di inserimento sono già dentro `<dialog>` (modifica, post-it), da qui il pannello in linea; "registra comunque" come terza opzione — non richiesto, e l'app calcolava già l'eccedenza come straordinario a posteriori.
**Nota:** i totali di straordinari di calendario, pagina Oggi ed export restano `totale giorno − 480` su **tutte** le voci. Con la divisione "straordinario" il totale del giorno non cambia, quindi i numeri coincidono; ma una voce `STRAORDINARIO` inserita a mano in un giorno sotto le 8h viene ancora sommata alle ore del giorno lì — incoerenza preesistente, non toccata.
**Impatto:** `src/lib/day-overflow.ts` e `src/lib/holidays.ts` (puri; i festivi erano in `calendario/page.tsx`), `src/app/actions.ts`, `src/components/OverflowPrompt.tsx`, `TimeEntryForm.tsx`, `EditButton.tsx` (ora riparte dalla voce salvata a ogni apertura: dopo una divisione riproponeva la durata digitata), `TaskCard.tsx` (data resa controllata). Il polling email crea `Task`, non voci: escluso.
